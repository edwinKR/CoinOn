...
class DashBoard extends React.Component {
  componentDidMount() {
    console.log('Hitting compDidMount???=====')
    this.createChart()
  }

  componentDidUpdate() {
    console.log('Hitting compDidUpdate=====')
    this.createChart()
  }

  createChart = () => {
    console.log('createChart() invoked!!')
    ...
    container
      .append('h1')
      .text('Who will win the 2018/19 Premier League Season?')
      ...
      svg
        ...
        .on('mousemove', d => {
          tip
            ...
            .style('opacity', '0.9')
            .html(
              `<div><strong>${d.name}</strong></div> <span>${
                d.votes
              } votes</span>`
            )
        })
    ...
    const pusher = new Pusher('36ccff46018e4c249f97', {
      cluster: 'us2',
      encrypted: true
    })
